...
class FileManager extends AppModel {
			...
			'rule' => array(
				...
				array(
					...
					'extensions' => array(
						...
						'tif', 'tiff', 'pdf','odt','ott','odm','html','oth','ods','ots','odg',
						'otg','odp','otp','odf','odb','oxt'
					)
				)
			)
		)
    ...
    function upload($data = null, $destFolder = null) 
	{
        ...
        if(!$this->validates()) 
		{
			$this->error = 'Chỉ được phép upload các file *.jpg, *.jpeg, *.gif, *.png, *.swf, *.flv, *.avi, *.wmv, *.wma, *.mp3, *.doc, *.docx, *.xls, *.xlsx, *.ppt, *.pptx, *.zip, *.rar, *.tif, *.tiff, *.pdf, *.odt, *.ott, *.odm, *.html, *.oth, *.ods, *.ots, *.odg, *.otg, *.odp, *.otp, *.odf, *.odb, *.oxt';
            return false;
        }
	...
	function upload_tinnhan($data = null, $destFolder = null) 
	{
        $this->set($data);
