...
//		array_push($conds, 'Nhanvanban.ngay_xem IS NULL');
	...
	public	function	vbden_gap()// xem lại chổ này khi dùng thêm bộ lọc
	{
		...
		if(!empty($this->request->data))
		{
			...
			$conds[] = array("Vanban.vb_gap" => 1, "Vanban.tinhtrang_duyet" => 1, "Vanban.chieu_di" => 1);
			if(!empty($this->request->data['Vanban']['keyword']))
			{
				$this->passedArgs['keyword'] = $this->data['Vanban']['keyword'];
				$conds["OR"]	= array(
			...
			if(!empty($this->request->data['capnhat_mucdo']) && $this->request->data['capnhat_mucdo'] == 'dahoanthanh')
			{
				$this->passedArgs['capnhat_mucdo'] = $this->request->data['capnhat_mucdo'];
				$this->loadModel('Ketquavanban');
				$this->Ketquavanban->bindModel(array(
					'belongsTo' => array("Vanban")
				),false);
				/*$vanban_ids = $this->Ketquavanban->find('all',array('fields' => 'vanban_id','conditions' => array(
																'nguoi_capnhat_id' => $this->Auth->user('nhanvien_id'),
																'capnhat_mucdo' => 10,
																'DATE_FORMAT(Ketquavanban.ngay_capnhat, "%Y-%m-%d")<= DATE_FORMAT(Vanban.vbgap_ngayhoanthanh, "%Y-%m-%d")'
																)));*/
				$vanban_ids = $this->Ketquavanban->find('all',array('fields' => 'vanban_id','conditions' => array(
																'nguoi_capnhat_id' => $this->Auth->user('nhanvien_id'),
																'capnhat_mucdo' => 10
				...
				$vb_ids = array();
				foreach($vanban_ids as $kq_vb)
				{
					if(!in_array($kq_vb["Ketquavanban"]["vanban_id"],$vb_ids))
						$vb_ids[] = $kq_vb["Ketquavanban"]["vanban_id"];
				}

				$conds["AND"] = array("Vanban.id" => $vb_ids);
			}
			else if(!empty($this->request->data['capnhat_mucdo']) && $this->request->data['capnhat_mucdo'] == 'chuahoanthanh') {
				$this->passedArgs['capnhat_mucdo'] = $this->request->data['capnhat_mucdo'];
				$this->loadModel('Ketquavanban');
				$this->Ketquavanban->bindModel(array(
					'belongsTo' => array("Vanban")
				...
				foreach($vanban_ids as $kq_vb)
				{
					if(!in_array($kq_vb["Ketquavanban"]["vanban_id"],$ht_ids))
						$ht_ids[] = $kq_vb["Ketquavanban"]["vanban_id"];
				}

				$vanban_ids = $this->Ketquavanban->find('all',array('fields' => 'vanban_id','conditions' => array(
																'nguoi_capnhat_id' => $this->Auth->user('nhanvien_id'),
																'capnhat_mucdo <' => 10,
																'vanban_id NOT' => $ht_ids
																)));
				...
				foreach($vanban_ids as $kq_vb)
				{
						$not_ids[] = $kq_vb["Ketquavanban"]["vanban_id"];
				}
				$this->Ketquavanban->unbindModel(array("belongsTo" => array("Vanban")),false);
				$this->loadModel("Nhanvanban");
				$this->Nhanvanban->bindModel(array(
					"belongsTo" => array('Vanban' => array('foreignKey' => 'vanban_id')
				)));
				$vanban_ids = array();
				$vanban_ids = $this->Nhanvanban->find('all',array('fields' => 'Nhanvanban.vanban_id','conditions' => array(
																'Nhanvanban.nguoi_nhan_id' => $this->Auth->user('nhanvien_id'),
																'Nhanvanban.vanban_id NOT' => $not_ids,
																'Nhanvanban.vanban_id NOT' => $ht_ids,
																'Nhanvanban.vanban_id = Vanban.id',
																"Vanban.vb_gap" 					 => 1,
																"Vanban.tinhtrang_duyet" 	 => 1,
																"Vanban.chieu_di" 				 => 1
																)));
				foreach($vanban_ids as $kq_vb)
				{
						$vb_ids[] = $kq_vb["Nhanvanban"]["vanban_id"];
				}
				$conds["AND"] = array("Vanban.id" => $vb_ids);
			}
			else
			{
				$this->loadModel("Nhanvanban");
				$vanban_ids = array();
				$vanban_ids = $this->Nhanvanban->find('all',array('fields' => 'vanban_id','conditions' => array(
																'nguoi_nhan_id' => $this->Auth->user('nhanvien_id'),
																)));
				foreach($vanban_ids as $kq_vb)
				{
						$vb_ids[] = $kq_vb["Nhanvanban"]["vanban_id"];
		...
		}elseif(isset($this->passedArgs))
		{
			...
			if( !empty($this->passedArgs['ngay_batdau'])
							&& !empty($this->passedArgs['ngay_ketthuc']))
			{
				$ngay_batdau = $this->passedArgs['ngay_batdau'];
				$ngay_ketthuc = $this->passedArgs['ngay_ketthuc'];
				$conds['Vanban.ngay_phathanh BETWEEN ? AND ?'] = array( $ngay_batdau,  $ngay_ketthuc);
			}
			if(!empty($this->passedArgs['capnhat_mucdo']) && $this->passedArgs['capnhat_mucdo'] == 'dahoanthanh') {
				$this->loadModel('Ketquavanban');
				$this->Ketquavanban->bindModel(array(
					'belongsTo' => array("Vanban")
				),false);
			...
			else if(!empty($this->passedArgs['capnhat_mucdo']) && $this->passedArgs['capnhat_mucdo'] == 'chuahoanthanh') {
				$this->loadModel('Ketquavanban');
				$this->Ketquavanban->bindModel(array(
					'belongsTo' => array("Vanban")
				),false);
				$vanban_ids = $this->Ketquavanban->find('all',array('fields' => 'vanban_id','conditions' => array(
																...
																'OR' => array(
																	'capnhat_mucdo <' => 10,
																	array(
																		'Ketquavanban.ngay_capnhat > Vanban.vbgap_ngayhoanthanh',
																		'capnhat_mucdo' => 10
																	)
																)
																)));
	...
	/*public	function	vbden_gap()// xem lại chổ này khi dùng thêm bộ lọc
	{
		...
		if(empty($this->request->data['capnhat_mucdo']))
		{
			$this->loadModel('Nhanvanban');
			$this->Nhanvanban->bindModel(array(
									belongsTo'	=>	array('Vanban')
			...
			$this->Nhanvanban->Vanban->bindModel(array(
				hasMany'	=>	array('Ketquavanban' => array('className'	=>	'Ketquavanban','foreignKey' => 'vanban_id',
										conditions' => array('nguoi_capnhat_id' => $this->Auth->user('nhanvien_id')))
								)
			), false);
			...
			if(!empty($this->request->data))
			{
				$ngay_batdau = $this->Bin->vn2sql($this->request->data['Vanban']['ngay_batdau']);
				$ngay_ketthuc = $this->Bin->vn2sql($this->request->data['Vanban']['ngay_ketthuc']);
				$keyword = $this->data['Vanban']['keyword'];
				if(!empty($this->request->data['Vanban']['keyword']))
				{
					$this->passedArgs['keyword'] = $this->data['Vanban']['keyword'];
					$conds["OR"]	= array(
				...
				var_dump($ngay_batdau);
				var_dump($ngay_ketthuc);
				var_dump($keyword);
				pr($this->request->data['capnhat_mucdo']);die();

				if($this->request->data['capnhat_mucdo'] == 'dahoanthanh' && !$ngay_batdau && !$ngay_ketthuc && empty($keyword) )
				{
					$ds = $this->Vanban->query("
												select a.*, b.*, c.*, d.ten_phong
												from vanban_thongtin a, vanban_nhan b, vanban_nhanketqua c, nhansu_phong d
												where a.chieu_di = 1 and a.vb_gap = 1 and a.tinhtrang_duyet = 1
												and b.nguoi_nhan_id = ".$this->Auth->user('nhanvien_id')."
												and a.id = b.vanban_id
												and a.id = c.vanban_id and c.capnhat_mucdo = 10
												and c.ngay_capnhat <= a.vbgap_ngayhoanthanh
			...
			}elseif(isset($this->passedArgs))
			{
				...
				if(!empty($this->passedArgs['capnhat_mucdo']) && !empty($this->data['Ketquavanban']['capnhat_mucdo']))
					{
						if($this->passedArgs['capnhat_mucdo'] == 'dahoanthanh')
							array_push($conds, 'Ketquavanban.capnhat_mucdo = 10');
						else
							array_push($conds, 'Ketquavanban.capnhat_mucdo < 10');
					}
			}
...

	{
...

		{
			...
			///////////
			/* Lấy thông tin để save Log */
			$vb_log = array();
			$info = $this->Vanban->find('first', array('conditions' => array('Vanban.id' => $id)));
			$vb_log['id'] = NULL;
			$vb_log['vanban_id'] =  $info['Vanban']['id'];
			$vb_log['nguoi_duyet_id'] = $info['Vanban']['nguoi_duyet_id'];
			$vb_log['noidung_duyet'] = $info['Vanban']['noidung_duyet'];
			$vb_log['nguoi_trinh_id'] = $info['Vanban']['nguoi_trinh_id'];
			$vb_log['noidung_trinh'] = $info['Vanban']['noidung_trinh'];
			$vb_log['phongchutri_id'] = $info['Vanban']['phongchutri_id'];
			$vb_log['vb_gap'] = $info['Vanban']['vb_gap'];
			$vb_log['vbgap_ngayhoanthanh'] = $info['Vanban']['vbgap_ngayhoanthanh'];
			$vb_log['chuyen_nguoiduyet'] = $info['Vanban']['chuyen_nguoiduyet'];
			$vb_log['gd_chidao'] = $info['Vanban']['gd_chidao'];
			$vb_log['uy_quyen'] = $info['Vanban']['uy_quyen'];
			$vb_log['ngay_duyet'] = $info['Vanban']['ngay_duyet'];
			$vb_log['nhanvienid_edit'] = $this->Auth->User('nhanvien_id');
			$vb_log['ngay_edit'] = date("Y-m-d H:i:s");
			$vb_log['opt'] = 0; // sửa văn bản đến của chức năng văn thư
			//////////
...

			{
				...
				$f = true;
				//Lưu log
				$this->loadModel('Vanbanlog');
				if(!$this->Vanbanlog->save($vb_log))
				{
					$f = false; break;
				}
				// save nguoi nhan
...

	{
...

		{
			...
			/* Lấy thông tin để save Log */
			$vb_log = array();
			$info = $this->Vanban->find('first', array('conditions' => array('Vanban.id' => $ids)));
			$vb_log['id'] = NULL;
			$vb_log['vanban_id'] =  $info['Vanban']['id'];
			$vb_log['nguoi_duyet_id'] = $info['Vanban']['nguoi_duyet_id'];
			$vb_log['noidung_duyet'] = $info['Vanban']['noidung_duyet'];
			...
			$vb_log['nhanvienid_edit'] = $this->Auth->User('nhanvien_id');
			$vb_log['ngay_edit'] = date("Y-m-d H:i:s");
			$vb_log['opt'] = 10; // xóa văn bản của văn thư
			$this->loadModel('Vanbanlog');
			//pr($vb_log);die();
			if(!$this->Vanbanlog->save($vb_log))
			{
...

	{
		...
		$files_trinh = $this->Filevanbanduyet->find('all', array('conditions' => array('Filevanbanduyet.vanban_id' => $data['Vanban']['id'])));
		if(empty($data))
		...
		$this->set(compact('nv','phong_nhan', 'nguoi_nhan', 'is_mobile','check','phong_chutri','arr_dsphong','arr_nguoiduyet','kq_vanban','files_trinh'));
...
function	getfile_ggd($id, $nv, $token, $id_vanban, $timestamp = null)

	{
...

		{
			...
			if($duration > 120)
				...
				throw new InternalErrorException('Bạn không được phép download văn bản này. Vui lòng chọn văn bản khác.');
			$nv1 = $nv;
			$van_ban = $this->Vanban->find('first', array('conditions' => array('Filevanban.id' => $id, 
																				'OR' => array('Vanban.nguoi_nhap_id' => $nv1,
																				              'Vanban.nguoi_duyet_id' => $nv1,
																							  'Vanban.nguoi_trinh_id' => $nv1,
																							  'Nhanvanban.nguoi_nhan_id' => $nv1)
																		 )
														   ));
			//var_dump($van_ban);die();
			if(empty($van_ban))
...

	{
		...
		//{
			...
			$conds[] = array('Vanban.tinhtrang_duyet' => 1,
								'OR' => array('Nhanvanban.nguoi_nhan_id'	=>	$this->Auth->user('nhanvien_id'),
											  'Vanban.nguoi_trinh_id'	=>	$this->Auth->user('nhanvien_id'),
											  'Vanban.nguoi_duyet_id'	=>	$this->Auth->user('nhanvien_id')
								));
			/*$conds = array( 'Nhanvanban.nguoi_nhan_id'	=>	$this->Auth->user('nhanvien_id'),
...

		{
			...
			if(!empty($this->request->data['Vanban']['so_hieu']))

			{
				$this->passedArgs['so_hieu'] = $this->data['Vanban']['so_hieu'];
				$conds[] = array("Vanban.so_hieu LIKE"	=>	"%" . $this->request->data['Vanban']['so_hieu'] . "%");
			}
			if(!empty($this->request->data['Vanban']['trich_yeu']))

			{
				$this->passedArgs['trich_yeu'] = $this->data['Vanban']['trich_yeu'];
				$conds[] = array("Vanban.trich_yeu LIKE"	=>	"%" . $this->request->data['Vanban']['trich_yeu'] . "%");
			}
...

		{
			...
			if(!empty($this->passedArgs['so_hieu']))

			{

				$conds[] = array("Vanban.so_hieu LIKE"	=>	"%" . $this->passedArgs['so_hieu'] . "%");

			}
			if(!empty($this->passedArgs['trich_yeu']))

			{

				$conds[] = array("Vanban.trich_yeu LIKE"	=>	"%" . $this->passedArgs['trich_yeu'] . "%");
...

	{
		...
		if($this->Auth->User('phong_id')== 107 || $this->Auth->User('phong_id')== 108 ) // phát sinh từ khi chia nhóm P.THHC
			$donvi_id = 20;
		$conds = array( 'Nhanvanban.nguoi_nhan_id'	=>	$this->Auth->user('nhanvien_id'),
						'Vanban.tinhtrang_duyet' => 1,
						'Vanban.phongchutri_id' => $donvi_id);
		if(!empty($this->request->data))
		{
			...
			if($this->request->data['Vanban']['vb_gap'] != '')
			{
					$this->passedArgs['vb_gap'] = $this->data['Vanban']['vb_gap'];
					$conds["Vanban.vb_gap"] = $this->request->data['Vanban']['vb_gap'];
			}
			if( !empty($this->request->data['Vanban']['ngay_batdau'])
...

		{
			...
			if(isset($this->passedArgs['vb_gap']))
			{
				$conds["Vanban.vb_gap"] = $this->passedArgs['vb_gap'];
			}
			if( isset($this->passedArgs['ngay_batdau'])
...

		{
			...
			$vb_gap	= array("1"	=>	'Văn bản gấp(có deadline)',
								"0"	=>	'Văn bản thường');
			$this->set(compact('dsphong','vb_gap'));
...
//		array_push($conds, 'Nhanvanban.ngay_xem IS NULL');
	...
	public	function 	trinh_vanban($id = null)//trình văn bản đến (Trưởng VPTH)
	{
		...
		if(!empty($this->request->data))
		{
			...
			if ($chuyen_bypass == 0) // ko phải văn bản mang tính chất thông báo
			{
				...
				//pr($this->request->data['Nhanvanban']);die();
				///////////
			...
			{
				$ngay_duyet = date("Y-m-d H:i:s"); // Thêm ngày duyệt văn bản
				$ds = $this->request->data['Vanban']['nv_selected'];
				$nguoinhan = explode(",", $ds);
				...
				if($this->Vanban->updateAll(
							...
							array('tinhtrang_duyet' => 1, 'noidung_trinh' => "'" . $noidung_trinh . "'", 'noidung_duyet' => "'" . $noidung_trinh . "'",'nguoi_trinh_id' => "'". $nguoi_trinh_id . "'",'nguoi_duyet_id' => "'". $nguoi_trinh_id . "'" ,'nguoi_duyet' => "'". $nguoi_duyet . "'" ,'chuyen_bypass' => 1,'phongchutri_id' => "'" . $phongchutri_id ."'",'phongphoihop_id' => "'" . ltrim($phong_phoihop_id,',') ."'", 'ngay_duyet' => "'" . $ngay_duyet."'"),
							array('Vanban.id' 	=> $id )
							)
						)
				{
...

	{
		if($this->Auth->User('nhanvien_id') == 683 || $this->Auth->User('nhanvien_id') == 744)//Chị Thu, Anh Dũng
		{
			...
			$ds_phong = $this->Phong->find('list', array('conditions' => array('id' => 107, 'enabled' => 1), 'fields' => array('id', 'id')));
			//pr($ds_phong);die();
			$this->Vanban->recursive = 2;
	...
	function	edit_vanbantrinh($id = null)
	{
		...
		if(!empty($this->request->data))
		{
			...
			$vb_log['nhanvienid_edit'] = $this->Auth->User('nhanvien_id');
			$vb_log['ngay_edit'] = date("Y-m-d H:i:s");
			$vb_log['opt'] = 1; // sửa văn bản trình
			$dataSource = $this->Vanban->getDataSource();
			$dataSource->begin();
            if ($this->Vanban->save($this->request->data))
			{
				...
				$this->loadModel('Ketquavanban');
				$result_old = $this->Ketquavanban->find('all', array('conditions' => 'vanban_id=' . $this->data['Vanban']['id']));
				if($f)
					if(!empty($result_old))
						foreach($result_old as $k)
						{
							//Lấy dl để lưu log
							$n = array();
							$n['id'] = NULL;
							$n['nguoi_nhan_id'] = $k['Ketquavanban']['nguoi_nhan_id'];
							$n['nguoi_capnhat_id'] = $k['Ketquavanban']['nguoi_capnhat_id'];
							$n['vanban_id'] = $k['Ketquavanban']['vanban_id'];
							$n['nguoi_nhan_id'] = $k['Ketquavanban']['nguoi_nhan_id'];
							$n['ngay_capnhat'] = $k['Ketquavanban']['ngay_capnhat'];
							$n['noidung_capnhat'] = $k['Ketquavanban']['noidung_capnhat'];
							$n['capnhat_mucdo'] = $k['Ketquavanban']['capnhat_mucdo'];
							$n['nhanvienid_edit'] = $this->Auth->User('nhanvien_id');
							$n['ngay_edit'] = date("Y-m-d H:i:s"); 
							$n['opt'] = 1; //sửa văn bản trình
							$this->loadModel('Ketquavanbanlog');
							if(!$this->Ketquavanbanlog->save($n))
							{
								$f = false;	break;
							}
						}
...

	{
		...
		if(!empty($this->request->data))
		{
			...
			/*$chuyen_nguoiduyet = isset($this->request->data['Vanban']['chuyen_nguoiduyet']) ? 1 : 0;
			$importance = isset($this->request->data['Vanban']['importance']) ? 1 : 0;
			$vb_gap = isset($this->request->data['Vanban']['vb_gap']) ? 1 : 0;*/
			...
			if($this->request->data['Vanban']['pgd_khac'] == 1 && !empty($this->request->data['Vanban']['nguoi_duyetkhac_id']) )
				$this->request->data['Vanban']['nguoi_duyet_id'] = $this->request->data['Vanban']['nguoi_duyetkhac_id'];
			//pr($this->request->data);die();
			$ds = $this->request->data['Vanban']['nv_selected'];
			$nguoinhan = explode(",", $ds);
			$this->loadModel('Nhanvanban');
				...
				if($this->Auth->User('nhanvien_id') == 681)
					$up_date = $this->Vanban->updateAll(
							array('tinhtrang_duyet' => 1, 
								  ...
								  'importance' => $importance, // thuộc tính này dành cho GĐ VTĐN
								  'vb_gap' => $vb_gap, 
								  'vbgap_ngayhoanthanh' => "'".$vbgap_ngayhoanthanh."'", 
								  'donviduyet_id' => $donviduyet_id, 
								  'ngay_duyet' =>  "'".$ngay_duyet. "'"),
							array('Vanban.id' 	=> $id )
							);
				else
			...
			} else //chuyển cho PGĐ duyệt
			{
				...
				if($this->Auth->User('nhanvien_id') == 681)
					$up_date = $this->Vanban->updateAll(
								array('nguoi_duyet_id' => "'" . $nguoi_duyet_id . "'" , 
...

		{
			...
			if ($this->Auth->User('donvi_id') == '')
			{
				$arr_nvnvb =  in_array(784,$arr_nvnvb) ? array_diff($arr_nvnvb,array(784)) : $arr_nvnvb;
				$arr_nvnvb =  in_array(681,$arr_nvnvb) ? array_diff($arr_nvnvb,array(681)) : $arr_nvnvb;
				$nhanviennhan = $this->Nhanvien->find('all', array('conditions' => array('Nhanvien.id in ('.implode(',',$arr_nvnvb).')','nguoi_quanly = 1', 'tinh_trang = 1', 'Phong.thuoc_phong' => $this->Session->read('Auth.User.donvi_id'), 'Chucdanh.nhomquyen_id in (14,15)'),'fields' => array('id','User.fullname'), 'order' => array('Chucdanh.nhomquyen_id ASC','Nhanvien.stt ASC'),));
				/*if(!in_array(784,$arr_nvnvb)) array_push($arr_nvnvb,784);
				if(!in_array(681,$arr_nvnvb)) array_push($arr_nvnvb,681);
				$pgd_konhan = $this->Nhanvien->find('all', array('conditions' => array('Nhanvien.id not in ('.implode(',',$arr_nvnvb).')','nguoi_quanly = 1', 'tinh_trang = 1', 'Phong.thuoc_phong' => $this->Session->read('Auth.User.donvi_id'), 'Chucdanh.nhomquyen_id in (14,15)'),'fields' => array('id','User.fullname'), 'order' => array('Chucdanh.nhomquyen_id ASC','Nhanvien.stt ASC'),));*/
				$pgd_konhan = $this->Nhanvien->find('all', array('conditions' => array('Nhanvien.id not in (784,681)','nguoi_quanly = 1', 'tinh_trang = 1', 'Phong.thuoc_phong' => $this->Session->read('Auth.User.donvi_id'), 'Chucdanh.nhomquyen_id in (14,15)'),'fields' => array('id','User.fullname'), 'order' => array('Chucdanh.nhomquyen_id ASC', 'Nhanvien.stt ASC'),));
				$pgd_konhan = Set::combine($pgd_konhan, '{n}.Nhanvien.id', '{n}.User.fullname');
			}
			else
			...
			if(empty($files))
				$files = $this->Filevanban->find('all', array('conditions' => array('Filevanban.vanban_id' => $data['Vanban']['id_vanban'] )));
			$this->set(compact('nhanviennhan','files','is_mobile','pgd_konhan'));
	...
	function	edit_vanbanduyet($id = null)
	{
		...
		if(!empty($this->request->data))
		{
			...
			$vb_log['nhanvienid_edit'] = $this->Auth->User('nhanvien_id');
			$vb_log['ngay_edit'] = date("Y-m-d H:i:s");
			$vb_log['opt'] = 2; // sửa văn bản duyệt
				...
				if($this->Auth->User('nhanvien_id') == 681)
					$up_date = $this->Vanban->updateAll(
											array('noidung_duyet' => "'" . $noidung_duyet . "'",
												  'phongphoihop_id' => "'" . $phongphoihop_id . "'",
												  'importance' => $importance, // thuộc tính này chỉ dùng cho GĐ VTĐN
												  'vb_gap' => $vb_gap,
												  'vbgap_ngayhoanthanh' => "'".$vbgap_ngayhoanthanh."'",
												  'ngay_duyet' => "'".$ngay_duyet."'"),
											array('Vanban.id' 	=> $id )
	...
	function	edit_vanbanchuyen($id = null)
	{
		...
		if(!empty($this->request->data))
		{
			...
			$vb_log['nhanvienid_edit'] = $this->Auth->User('nhanvien_id');
			$vb_log['ngay_edit'] = date("Y-m-d H:i:s");
			$vb_log['opt'] = 3; // sửa văn bản chuyển
			$this->loadModel('Vanbanlog');
				//pr($vb_log);die();
				if(!$this->Vanbanlog->save($vb_log))
				{
	...
	function	up_result()//cập nhật kết quả thực hiện công việc theo văn bản

	{
...

		{
			...
			if(!in_array($data['Vanban']['nguoi_duyet_id'],$ds_nguoinhan))
				array_push($ds_nguoinhan, $data['Vanban']['nguoi_duyet_id']);
			if(!in_array($data['Vanban']['nguoi_trinh_id'],$ds_nguoinhan))
				array_push($ds_nguoinhan, $data['Vanban']['nguoi_trinh_id']);
			...
			$ds_nhan = array(681,683,$data['Vanban']['nguoi_duyet_id']);
			$progress = $this->Vanban->progress;
			$this->data = $data;
			$type = $this->passedArgs['type'];

 			$this->set(compact(array('progress','ds_nhan','type','id')));
...

	{
		...
		$this->Ketquavanban->bindModel(array(
			'belongsTo'	=>	array(
									'Vanban'	=>	array('foreignKey'	=>	'vanban_id'),
									'Nhanviennhan' => array('className'	=>	'Nhanvien','foreignKey' => 'nguoi_nhan_id'),
									'Nhanviencapnhat' => array('className'	=>	'Nhanvien','foreignKey' => 'nguoi_capnhat_id')
							),
		), false);
		$this->Ketquavanban->bindModel(array(
...

	{
		...
		$nv = $this->Auth->user('nhanvien_id');
		if($nv == 681 || $nv == 683)
			$data = $this->Ketquavanban->find('all',array('conditions' => array('Ketquavanban.vanban_id' => $id )));
		else
	...
	function	donvi_nv($nhanvien_id = null)//Lấy đơn vị của nhân viên
	{
		...
		{
			$thuoc_phong = $this->Nhanvien->find('all', array('fields' => array("Phong.id") , 'conditions' => array('Nhanvien.id' => $nv_id)));
			foreach($thuoc_phong as $phong)
			{
				$donvi = $phong["Phong"]["id"];
				if(!in_array($donvi,$donvi_id))
	...
	/// Trao đổi phản hồi văn bản
	function	traodoi_phanhoi($id = null)

	{
		$this->set('title_for_layout', 'Trao đổi, phản hồi văn bản');
		$this->Vanban->bindModel(array(
			'hasMany'	=>	array(
				'Filevanban'	=>	array('foreignKey'	=>	'vanban_id'),
				'Nhanvanban'	=>	array('foreignKey'	=>	'vanban_id'),
				'Traodoivanban'	=>	array('foreignKey' => 'vanban_id')
			)
		));
		...
		if(!empty($this->request->data))
		{
			...
			/*$this->loadModel('Nhanvanban');
			$nv_nhan = $this->Nhanvanban->find('list', array('conditions' => array('Nhanvanban.vanban_id' => $id),'fields' => 'nguoi_nhan_id'));
			foreach($nguoinhan as $n)
			{
				$f = in_array($n,$nv_nhan);
				if(!$f)
				{
					echo $f; die();
				}
			...
			//die();
			$this->loadModel('Traodoivanban');

			$this->request->data['Traodoivanban'] = array();

			if(!empty($nguoinhan))
			{
				foreach($nguoinhan as $n)
				{
					array_push($this->request->data['Traodoivanban'], array('nguoi_nhan_id' => $n));
				}
			}
			//pr($nguoinhan);die();
			$f = true;
			foreach($nguoinhan as $k => $v)
				{
					...
					$t['noidung_capnhat'] = $noidung_capnhat;
					//pr($t);die();
					if(!$this->Traodoivanban->save($t))
					{
						$f = false;	break;
					}
					//Đính kèm file trao đổi
					if(!empty($this->request->data['Filevanban']))
					{
						$f_bc['id'] = NULL;
						$f_bc['vanban_id'] = $id;
						...
						//$f_bc['kqbaocao_id'] = $t['id'];
						//pr($id); die();
						$kqbaocao_id = $this->Traodoivanban->find('all', array('conditions' => array('Traodoivanban.vanban_id' =>  $id,'Traodoivanban.nguoi_capnhat_id' => $this->Auth->User('nhanvien_id'))));
						//pr($kqbaocao_id);die();
						if(!empty($kqbaocao_id))
						{
							foreach($kqbaocao_id as $i)
							{
								$f_bc['kqbaocao_id'] = $i['Traodoivanban']['id'];
							}
						}
						$old = str_replace("/", DS, Configure::read('File.tmp'));
						$old = substr($old, 1, strlen($old)-1);
						...
						foreach($this->request->data['Filevanban'] as $item)
						{
							...
							if(copy(WWW_ROOT . $old . $f_bc['ten_moi'],  WWW_ROOT . $new . $f_bc['ten_moi']))
								unlink(WWW_ROOT . $old . $f_bc['ten_moi']);
							$this->loadModel('Filetraodoi');
							if (!$this->Filetraodoi->save($f_bc))
							{
								$f = false;break;
							}
						}
			...
			if($f)

				{
					...
					$this->Session->setFlash('Văn bản đã trao đổi, phản hồi thành công.', 'flash_success');

					$this->redirect('/vanban/index#vanban-traodoi');
...

				{
					...
					$this->Session->setFlash('Đã phát sinh lỗi trong khi Trao đổi Văn bản. Vui lòng thử lại.', 'flash_error');
...

		{
			...
			if(!$check)
				...
				throw new InternalErrorException('Bạn không có quyền Trao đổi, phản hồi văn bản này. Vui lòng thử lại.');

			//$progress = $this->Vanban->progress;
			$this->data = $data;
 			$this->set(compact(array('ds_nguoinhan')));

		}

	}
	////Văn bản đến có sự trao đổi, phản hồi
	public	function	vb_traodoiphanhoi()

	{
		...
		$this->Vanban->Traodoivanban->bindModel(array(
			'belongsTo'	=>	array('Nhanviencapnhat' => array('className'	=>	'Nhanvien','foreignKey' => 'nguoi_capnhat_id'),
    								'Nhanviennhan' => array('className'	=>	'Nhanvien','foreignKey' => 'nguoi_nhan_id')
							),
		), false);
		...
		$conds = array('Vanban.chieu_di'			=>	1,
					   'OR' => array('Traodoivanban.nguoi_capnhat_id'	=> $this->Auth->User('nhanvien_id'),
					   'Traodoivanban.nguoi_nhan_id'	=> $this->Auth->User('nhanvien_id') ));
		/*$conds = array('Vanban.chieu_di'			=>	1,
					   'Traodoivanban.nguoi_nhan_id'	=> $this->Auth->User('nhanvien_id'));*/
		if(!empty($this->request->data))
		{
	...
	///
	public	function	view_traodoi($id = null)

	{
		$this->set('title_for_layout', 'Kết quả trao đổi văn bản');

		$this->loadModel('Traodoivanban');

		$this->Traodoivanban->bindModel(array(
			'belongsTo'	=>	array(
									'Vanban'	=>	array('foreignKey'	=>	'vanban_id'),
									'Nhanviennhan' => array('className'	=>	'Nhanvien','foreignKey' => 'nguoi_nhan_id'),
									'Nhanviencapnhat' => array('className'	=>	'Nhanvien','foreignKey' => 'nguoi_capnhat_id')
		...
		$this->Traodoivanban->bindModel(array(
			...
			'hasMany' => array(
				...
				'Filetraodoi' => array('foreignKey' => 'kqbaocao_id',

					'conditions' => array('Filetraodoi.kqbaocao_id' => $id)
		...
		$nv = $this->Auth->user('nhanvien_id');
		$conds = array();
		$conds = array('Traodoivanban.id' => $id,
							'OR' => array('Traodoivanban.nguoi_nhan_id' => $nv,
											'Traodoivanban.nguoi_capnhat_id' => $nv)
			);
		$data = $this->Traodoivanban->find('first',array('conditions' => $conds));
			//$data = $this->Traodoivanban->find('first',array('conditions' => array('Traodoivanban.id' => $id,'Traodoivanban.nguoi_nhan_id' => $nv )));
		if(empty($data))
			throw new InternalErrorException('Bạn không có quyền xem kết quả trao đổi văn bản này. Vui lòng liên hệ quản trị để cấp quyền.');
		else
			$this->Traodoivanban->query("UPDATE vanban_traodoi SET ngay_xem='" . date("Y-m-d H:i:s") . "' WHERE id = " . $id . " AND nguoi_nhan_id=" . $this->Auth->user('nhanvien_id') . " AND ngay_xem IS NULL");
		$this->set(compact('data'));

		$this->set('title_for_layout', 'Kết quả xử lý văn bản');

	}
	//Download file trao đổi
	function	files_atttraodoi($id = null)

	{
		$this->loadModel('Filetraodoi');
		$vanban = $this->Filetraodoi->find('first', array('conditions' => array('Filetraodoi.id' => $id, 'nguoinhan_id' => $this->Auth->User('nhanvien_id'))));
		if(empty($vanban))
			$vanban = $this->Filetraodoi->find('first', array('conditions' => array('Filetraodoi.id' => $id, 'nguoibaocao_id' => $this->Auth->User('nhanvien_id'))));
		if(empty($vanban))
			throw new InternalErrorException('Bạn không được phép download văn bản này. Vui lòng chọn văn bản khác.');
		...
		$path = substr($path, 1, strlen($path)-1);
		$path = WWW_ROOT . $path;
		$file_moi = $vanban['Filetraodoi']['ten_moi'];
		$file_cu = $vanban['Filetraodoi']['ten_cu'];
		$file_contents = file_get_contents($path.$file_moi, true);
		$this->layout = null;
		Configure::write('debug',0);
		header('Content-Description: File Transfer');header('Content-Type: application/octet-stream');header('Content-Disposition: attachment; filename="'. $file_cu .'"');
...
/////////
	...
	public	function	vbgap_notice()
	{
		...
		//pr($this->Auth->User('donvi_id'));die();
		if($this->Auth->User('donvi_id')== '')
			$donvi_id = $this->Auth->User('phong_id');
		else
			$donvi_id = $this->Auth->User('donvi_id');
		...
		$conds = array( 'Vanban.chieu_di'			=>	1,
						...
						'Vanban.vb_gap'			=> 1,
						'Vanban.phongchutri_id' => $donvi_id,
						"DATE_FORMAT(Vanban.vbgap_ngayhoanthanh, '%Y-%m-%d') < "		=>	date('Y-m-d'));
		$this->loadModel('Ketquavanban');
		$vanban_ids = $this->Ketquavanban->find('all',array('fields' => 'vanban_id','conditions' => array(
																'nguoi_capnhat_id' => $this->Auth->user('nhanvien_id'),
																'capnhat_mucdo' => 10 
	...
	public	function	vbkhac_notice()
	{
		...
		$conds = array( 'Vanban.chieu_di'			=>	1,
						...
						'Vanban.vb_gap'			=> 0,
						'Vanban.phongchutri_id' => $donvi_id,
						"CheckNgayVB(Vanban.ngay_duyet,NOW()) >" => 2);
		$this->loadModel('Ketquavanban');
		$vanban_ids = $this->Ketquavanban->find('all',array('fields' => 'vanban_id','conditions' => array(
																'nguoi_capnhat_id' => $this->Auth->user('nhanvien_id'),
																'capnhat_mucdo' => 10
...
/////////// Viết chức năng này cho chị Thu - CVP
function	edit_vb_except($id = null)
	{
		if($this->Auth->User('nhanvien_id') != 683)
			throw new InternalErrorException('Bạn không có quyền hiệu chỉnh văn bản. Vui lòng liên hệ quản trị để cấp quyền.');
		$this->set('title_for_layout', 'Chỉnh sửa văn bản trình');
		$this->Vanban->bindModel(array(
			'hasMany'	=>	array(
				'Filevanban'	=>	array('foreignKey'	=>	'vanban_id'),
				'Filevanbanduyet'	=>	array('foreignKey'	=>	'vanban_id'),
		...
		if(!empty($this->request->data))
		{
			...
			$vb_log['nhanvienid_edit'] = $this->Auth->User('nhanvien_id');
			$vb_log['ngay_edit'] = date("Y-m-d H:i:s");
			$vb_log['opt'] = 9; // sửa văn bản trình
			$dataSource = $this->Vanban->getDataSource();
			$dataSource->begin();
            if ($this->Vanban->save($this->request->data))
			{
...
public	function	ketquakhac() // VB đến ko có deadline đã cập nhật kết quả xử lý

	{
		...
		$kq_vanban = $this->Ketquavanban->find('all',array('fields' => 'vanban_id','conditions' => array(
																'nguoi_nhan_id' => $this->Auth->user('nhanvien_id')
																)));
		//pr($kq_vanban);die();
		$vanban_ids = array();
		foreach($kq_vanban as $kq_vb)
		{
				$vanban_ids[] = $kq_vb["Ketquavanban"]["vanban_id"];
		}
		...
		//pr($vanban_ids);die();
			...
			if($this->Auth->User('chucdanh_id') == 2 || $this->Auth->User('nhanvien_id') == 683 )//GĐ VTĐN, CVP
			{
				...
				$conds = array('Vanban.vb_gap'	=>	0,
					   'Vanban.chieu_di'			=>	1,
					   'Vanban.tinhtrang_duyet' => 1,
					   'Nhanvien.phong_id'		=> $ds_phong,
					   'Vanban.id'				=> $vanban_ids
						);
			}
	...
	if(!empty($this->request->data))
	{
		...
		if($this->Auth->User('donvi_id') == '') // người duyệt là GĐ,PGĐ VTĐN
		{
			...
			if ($loai_donvi == 2 || $loai_donvi == 6)//BGĐ, PBCN, Tổ KSCL
			{
				...
				if($pctnew_id == 20) // code phát sinh khi tách TH-HC làm 2 nhóm
				{
					$nv_chucdanh[0]['b'] = array('id'=>'683');
					$nv_quyen[0]['b'] = array('id'=>'744');
					$nv_quyen[1]['b'] = array('id'=>'687');
				}
				if($pctnew_id == 32)
