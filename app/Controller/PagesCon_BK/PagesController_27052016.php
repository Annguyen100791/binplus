...
class PagesController extends AppController
{
	...
	public	function	notice()
	{
		...
		*/
		...
		//pr($vb_traodoi);die();
		/*$vb_notice = $this->Vanban->query("
												select count(vanban.id) as vb from (SELECT a.id
												FROM vanban_thongtin a
												left JOIN vanban_nhan b ON a.ID = b.vanban_id
												LEFT JOIN vanban_nhanketqua c ON a.id = c.vanban_id
												where a.vb_gap = 1
												and a.tinhtrang_duyet = 1
												and a.chieu_di = 1
												and a.phongchutri_id = ".$this->Auth->user('donvi_id')."
												and b.nguoi_nhan_id = ".$this->Auth->user('nhanvien_id')."
												and a.vbgap_ngayhoanthanh < now()
												and (c.ngay_capnhat > a.vbgap_ngayhoanthanh OR c.ngay_capnhat IS NULL)
		...
		/*$vbgap_notice = $this->Vanban->query("
   											...
   											and (c.capnhat_mucdo < 10 OR c.ngay_capnhat IS NULL)
												and a.id NOT IN (select distinct m.vanban_id from vanban_nhanketqua m where m.capnhat_mucdo = 10)
												GROUP BY a.id) vanban
												");
		//pr($vb_notice);die();
		...
		$vbgap_notice = $this->Vanban->find('all', array('conditions' => array('Nhanvanban.nguoi_nhan_id' => $this->Auth->user('nhanvien_id'),'Vanban.tinhtrang_duyet' => 1, 'Vanban.vb_gap' =>  1)));*/
		/*$vbgap_notice = $this->Vanban->query("
								select  b.id
