...
//pr($this->request->data); die();
			...
			{			
				...
				if(!empty($this->request->data['Congviec']['nv_selected'])){
					$nguoinhan = explode(",", $this->request->data['Congviec']['nv_selected']);
					$so_nguoiphoihop = sizeof($nguoinhan);
				}else{
					$so_nguoiphoihop = 0;
				}
				
				//if($nguoiphoihop==1){$so_nguoiphoihop=0;}else{$so_nguoiphoihop=$nguoiphoihop;}
				//Update congviecchinh_id = id Cong viec vua insert
				$this->Congviec->query("UPDATE congviec_thongtin SET congviecchinh_id=" . $id . ", so_nguoiphoihop = ".$so_nguoiphoihop." WHERE id=" . $id);
				
				//pr($nguoinhan); die();
				if(sizeof($nguoinhan)>0)
				{
				...
				//// Mười edit đoạn ni
				$id_vanban = $this->request->data['Congviec']['vanban_id'];
				if(!empty($id_vanban))
				////
				//if(isset($this->request->data['Congviec']['vanban_id']))
				{
					...
					if(sizeof($nguoinhan) > 0)
						...
						$nguoinhan = array($this->request->data['Congviec']['nguoinhan_id']);
				////
					//array_push($nguoinhan,$this->request->data['Congviec']['nguoinhan_id']);
					foreach($nguoinhan as $n)
					{
...
//	}
	...
	//Mười edit để update số người phối hợp
	public	function	edit($congviec_id = null)
	{
		if(!$this->check_permission('CongViec.khoitao'))
			throw new InternalErrorException('Bạn không có quyền giao việc. Vui lòng liên hệ quản trị để biết thêm chi tiết.');			
		...
		if(!empty($this->request->data))
		{
			...
			/*****************/
				...
				{					
					...
					{						
						$a['Congviec'] = array('id'			=>	NULL,
												  ...
												  'ngay_ketthuc'=>	$this->request->data['Congviec']['ngay_ketthuc'],
												  'ngay_giao'=>	date('Y-m-d H:i:s'),
												  'vanban_id'	=>	isset($this->request->data['Congviec']['vanban_id']),
												  'mucdo_hoanthanh'=>	0,
												  'giaoviec_tiep'	=>	0,
												  'loaicongviec_id'	=> $this->request->data['Congviec']['loaicongviec_id'] );
							//pr($a['Congviec']); die(); 
				...
				$this->Congviec->query("UPDATE congviec_thongtin SET so_nguoiphoihop = ".$so_nguoiphoihop." WHERE id=" . $congviec_id);
				$dataSource->commit();
				$this->Session->setFlash('Hiệu chỉnh thành công.', 'flash_success');	
			}
		}
...
//pr($data);die();
	...
	public	function	view($congviec_id = null,$refer = null)
	{
		...
		if(empty($data))
		{
			...
			if($data['Congviec']['nguoinhan_id']==$this->Auth->user('nhanvien_id')){
				$this->Congviec->updateAll(
						array('is_dadoc_comment' => 0),
						array('Congviec.congviecchinh_id' => $data['Congviec']['congviecchinh_id'],'Congviec.nguoinhan_id' => $this->Auth->user('nhanvien_id'))
					);
			}elseif($data['Congviec']['nguoi_giaoviec_id']==$this->Auth->user('nhanvien_id')){
				$this->Congviec->updateAll(
						array('giaoviec_comment' => 0),
						array('Congviec.congviecchinh_id' => $data['Congviec']['congviecchinh_id'],'Congviec.nguoi_giaoviec_id' => $this->Auth->user('nhanvien_id'))
					);
...

		{
			...
			if($this->CongviecComment->save($data)){
				...
				$this->Congviec->updateAll(
					...
					array('is_dadoc_comment' => 1,'giaoviec_comment' => 1),
