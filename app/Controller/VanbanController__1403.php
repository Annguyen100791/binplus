...
function	getfile_ggd($id, $nv, $token, $timestamp = null) 
...

	{
		...
		$conds = array();	
		$conds = array('Vanban.tinhtrang_duyet' => 1,'Vanban.phongchutri_id <>' => NULL);
		if(!empty($this->request->data))
		{
			//pr($this->Auth->User());die();
		...
		if(!empty($this->request->data))
		{
			...
			if ($chuyen_bypass == 0) // ko phải văn bản mang tính chất thông báo 
			{
				...
				if($this->Vanban->updateAll(
							...
							array('tinhtrang_duyet' => 2, 'noidung_trinh' => "'" . $noidung_trinh . "'", 'nguoi_duyet_id' => $nguoi_duyet_id, 'nguoi_duyet' => "'" . $nguoi_duyet . "'",'nguoi_trinh_id' => "'" . $nguoi_trinh_id . "'",'chuyen_bypass' => 0,'phongchutri_id' => "'" . $phongchutri_id ."'",'phongphoihop_id' => "'" . ltrim($phong_phoihop_id,',') ."'" ), 
