...
/*			$data =  $this->Congviec->find('threaded', array(
				...
				//Code lấy ra tổng số người phối hợp
					if(!empty($data)){
						...
						foreach($data as $it):	//pr($it); die();	
							$tam=$this->Congviec->find('count', array(
								'conditions' => array('not'=>array('parent_id'=>'Null'),'congviecchinh_id' => $it['Congviec']['id']), 
								'order' => 'id ASC',
								'recusive' => -1
							));	
							$data[$i]['HasChild']=$tam;
				...
				case 'commentmoi':
					...
					if(!empty($data)){
						...
						foreach($data as $it):	//pr($it); die();	
							$tam=$this->Congviec->find('count', array(
								'conditions' => array('not'=>array('parent_id'=>'Null'),'congviecchinh_id' => $it['Congviec']['id']), 
								//'conditions' => array('congviecchinh_id' => $it['Congviec']['id']), 
								'order' => 'id ASC',
								'recusive' => -1
							));	
							$data[$i]['HasChild']=$tam;
	...
	public	function	view($congviec_id = null,$refer = null)
	{
...

		{
			...
			$this->Congviec->updateAll(
					...
					array('Congviec.congviecchinh_id' => $data['Congviec']['congviecchinh_id'])
				);
