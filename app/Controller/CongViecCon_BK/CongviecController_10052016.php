...
class CongviecController extends AppController {
	...
	function	search($action = 'duocgiao')
	{
		...
		if($this->RequestHandler->isAjax())
		{
			switch($action)
			{
				...
				$this->Congviec->Vanban->bindModel(array(
								'belongsTo'	=>	array('Phongchutri' => array('className'	=>	'Phong','foreignKey' => 'phongchutri_id')
												),
							), false);
				...
				$this->Congviec->Vanban->Phongphoihop->bindModel(array(
...

	{
		...
		$table = array(
			...
			array('label' => __('Trích yếu VB'), 'filter' => true),
		...
		if(!empty($ds)):
			...
			foreach ($ds as $d):
				...
				foreach($d['Vanban']['Phongphoihop'] as $key => $phongph):



					if($key != count($d['Vanban']['Phongphoihop']) -1)
				...
				$this->PhpExcel->addTableRow(array(
					...
					$d['Vanban']['Phongchutri']['ten_phong'],
		...
		foreach($ds as $d){
			...
			if(isset($d['Vanban']['Phongphoihop']) && !empty($d['Vanban']['Phongphoihop']))
			...
			$this->PhpExcel->getActiveSheet()->getRowDimension($i)->setRowHeight('80%');
	...
	{
		$this->Congviec->unbindModel(array('NguoiNhanviec','NguoiGiaoviec'));
...
//pr($this->request->data); die();
			...
			{			
				...
				//Update congviecchinh_id = id Cong viec vua insert
				$this->Congviec->query("UPDATE congviec_thongtin SET congviecchinh_id=" . $id . " WHERE id=" . $id);
				
				//Them nguoi nhan phoi hop
				$nguoinhan = explode(",", $this->request->data['Congviec']['nv_selected']);			
				...
				if(!empty($nguoinhan))
				{
					foreach($nguoinhan as $n)
					{
						$a['Congviec'] = array('id'			=>	NULL,
											  ...
											  'ngay_batdau'	=>	$this->request->data['Congviec']['ngay_batdau'],
											  'ngay_ketthuc'=>	$this->request->data['Congviec']['ngay_ketthuc'],
											  'ngay_giao'=>	date('Y-m-d H:i:s'),
											  'vanban_id'	=>	isset($this->request->data['Congviec']['vanban_id']) ? $this->request->data['Congviec']['vanban_id'] : null,
											  'mucdo_hoanthanh'=>	0,
											  'giaoviec_tiep'	=>	0,
											  'loaicongviec_id'	=> $this->request->data['Congviec']['loaicongviec_id'] );
				...
				//Thêm quyền đọc văn bản cho người nhận
				if(isset($this->request->data['Congviec']['vanban_id']))
				{
					array_push($nguoinhan,$this->request->data['Congviec']['nguoinhan_id']);
					foreach($nguoinhan as $n)
					{
						if(!$this->vanbangiaoviec($n, $this->request->data['Congviec']['vanban_id']))
						{
		...
		$tinhchat = $this->Tinhchatcongviec->find('list', array('conditions' => 'enabled=1', 'fields' => array('id', 'ten_tinhchat')));		//pr($tinhchat);die();

		$loai_congviec	= array("1"	=>	'Theo dõi và báo cáo',
								"0"	=>	'Chỉ view');
	...
	public	function	add_dachay_Ha_luu_27042016($vanban_id = null)
	{
		$this->Congviec->unbindModel(array('NguoiNhanviec','NguoiGiaoviec'));
...
//pr($this->request->data); die();
			if($this->Congviec->save($this->request->data))
			{
				...
				//Them quyen doc vb cho nguoi chu tri
				if(isset($this->request->data['Congviec']['vanban_id']))
				{
...

	{
		...
		$this->Congviec->bindModel(
			...
			array(
				...
				'belongsTo' => array(
					...
					'Vanban'	=>	array('fields' => array('id','trich_yeu', 'so_hieu', 'ngay_phathanh', 'noi_gui')),
...

		{
			...
			/*****************/
				...
				//Xóa các người nhận cũ 				



				$this->Congviec->deleteAll($conditions = array(



						'Congviec.parent_id' => $congviec_id,



						'Congviec.nguoi_giaoviec_id' => $this->Auth->user('nhanvien_id')
				...
				{					
					...
					{						
						...
						$a['Congviec'] = array('id'			=>	NULL,
												  ...
												  'vanban_id'	=>	$this->request->data['Congviec']['vanban_id'],
...
//pr($data);die();
		...
		$this->set(compact('tinhchat', 'nv','loai_congviec','data'));
		/*************** Hà thêm *****************/

		$dsnguoinhan = $this->Congviec->find('all', array('conditions' => array('Congviec.parent_id' => $congviec_id, 'Congviec.nguoi_giaoviec_id' => $this->Auth->user('nhanvien_id'),'Congviec.nguoinhan_id !='. $this->Auth->user('nhanvien_id')), 'recursive' => 1));
		//pr($dsnguoinhan);//die();



		$this->set(compact('dsnguoinhan'));
...

	{
			...
			foreach($nhanviennhan as $v)
			{
				...
				//{
					...
					$tmp = array(
						...
						'loaicongviec_id' 	=> $this->request->data['Nguoinhanviec']['loaicongviec_id']
					);
					//pr($tmp);die();
					$ds[$key] = $tmp;
...

		{
			$loai_congviec	= array("1"	=>	'Theo dõi và báo cáo',
								"0"	=>	'Chỉ view');
			$this->set(compact('loai_congviec'));
		}
	}
	...
	public	function	dagiao()
	{
		...
		if(!$this->check_permission('CongViec.khoitao'))
			throw new InternalErrorException('Bạn không có quyền giao việc. Vui lòng liên hệ quản trị để biết thêm chi tiết.');
		/******** Hà thêm để lấy trích yếu văn bản********************/
			$this->Congviec->bindModel(array(
			...
			switch($opt)
			{
				case 'all':
					...
					if(!empty($data)){
						...
						$i=0;
						foreach($data as $it):	//pr($it); die();	//, 'nguoinhan_id !='=> $this->Auth->user('nhanvien_id')					
							$tam=$this->Congviec->find('count', array(
								'conditions' => array('not'=>array('parent_id'=>'Null'),'congviecchinh_id' => $it['Congviec']['congviecchinh_id']), 
								'order' => 'Congviec.id ASC',
								'recusive' => -1
							));	
				...
				case 'commentmoi':
					...
					if(!empty($data)){
						...
						$this->Session->setFlash('Không có công việc nào đã giao có thảo luận mới.', 'flash_attention');
					}

					$this->set(compact('data'));
					$this->render('dagiao_commentmoi');
					break;	
		...
		}else
		{
			...
			/*$this->Congviec->bindModel(array(
				'hasMany' => array(	
					...
					'DsPhoiHop'	=>	array(	'className' => 'Congviec',

											'foreignKey' => 'parent_id',

											'fields'=>array('nguoinhan_id','mucdo_hoanthanh','ngay_capnhat')
...
/*			$data =  $this->Congviec->find('threaded', array(
					'conditions' => array('Congviec.nguoinhan_id' => $this->Auth->user('nhanvien_id'))
				));*/	
				//Code lấy ra tổng số người phối hợp
					if(!empty($data)){
						$i=0;
						foreach($data as $it):	//pr($it); die();
							$cond = array('congviecchinh_id' => $it['Congviec']['congviecchinh_id'], 'nguoinhan_id !=' => $this->Auth->user('nhanvien_id'));
							$tam=$this->Congviec->find('count', array(
								'conditions' => $cond, 
								'order' => 'id ASC',
								'recusive' => -1
					...
					/*
					//Code lấy ra danh sách người phối hợp
					if(!empty($data)){	
							$i=0;
							foreach($data as $it):	//pr($it); die();
								if(!empty($it['DsPhoiHop'])){
								$arr = array();
									foreach($it['DsPhoiHop'] as $child){
										$tam=$this->Nhanvien->find('first', array(
											'conditions' => array('Nhanvien.id' => $child['nguoinhan_id']),
											'fields'	=> 'full_name'

										));	
										array_push($arr,$tam['Nhanvien']['full_name']);
									}
									$data[$i]['HasChild'] = $arr;	
								}	
								$i++;
						...
						/*$i=0;
						foreach($data as $it):	//pr($it); die();	
							$tam=$this->Congviec->find('all', array(
								'conditions' => array('parent_id' => $it['Congviec']['parent_id'], 'nguoinhan_id !=' => $this->Auth->user('nhanvien_id') ), 
				...
				case 'commentmoi':
					...
					if(!empty($data)){
						...
						$this->Session->setFlash('Không có công việc nào được giao có thảo luận mới.', 'flash_attention');
					}

					$this->set(compact('data'));
					$this->render('duocgiao_commentmoi');
					break;					
	...
	public	function	view($congviec_id = null,$refer = null)
	{
		...
		if(empty($data))
		{
			...
			$this->Congviec->updateAll(
					...
					array('Congviec.id' => $congviec_id,'Congviec.nguoinhan_id' => $this->Auth->user('nhanvien_id'))
				);
	...
	public	function	chon_vanbannhan()
...
//		pr($detail);die();
...

	{
		...
		$this->loadModel('CongviecComment','Congviec');
...

		{
			...
			if($this->CongviecComment->save($data)){
				...
				//Update tình trang da_doc_comment ve 1: Chua doc, có comment moi; 0: da doc het comment

				$this->Congviec->updateAll(

					array('is_dadoc_comment' => 1),

					array('Congviec.congviecchinh_id' => $this->request->data['congviec_id'] )
	...
	public	function	view_nguoinhan($congviec_id)
	{
		
		$nguoinhan = $this->Congviec->find('all', array(
			'conditions'	=>	array(
						"Congviec.congviecchinh_id"	=>	$congviec_id,
