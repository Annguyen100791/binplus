			...
			switch($opt)
			{
					...
					if(!empty($data)){
						...
						foreach($data as $it):	//pr($it); die();	
							$tam=$this->Congviec->find('count', array(
								'conditions' => array('not'=>array('Congviec.parent_id'=>'Null'),'Congviec.parent_id' => $it['Congviec']['id']), 
				...
				case 'instant':
					...
					if(!empty($data)){
						...
						foreach($data as $it):	//pr($it); die();	
							$tam=$this->Congviec->find('count', array(
								'conditions' => array('not'=>array('parent_id'=>'Null'),'parent_id' => $it['Congviec']['id']), 
								'order' => 'id ASC',
								'recusive' => -1
							));	
							$data[$i]['HasChild']=$tam;
