<?php
class TinhchatvanbanController extends AppController {
	...
	public function beforeFilter() {
        parent::beforeFilter();
		if(!$this->check_permission('HeThong.tinhchatvanban'))
			throw new InternalErrorException('Bạn không có quyền quản lý Tính chất văn bản. Vui lòng liên hệ quản trị để biết thêm chi tiết.');
	}
	...
	public function index()
	{
		...
		$ds = $this->paginate('Tinhchatvanban', $conds);
		if(empty($ds))
		{
	...
	{
		$data = $this->Tinhchatvanban->read(array('enabled'), $id);
		$status = 0;
		$success = 0;
		
		if(!empty($data))
		{
			if($data['Tinhchatvanban']['enabled'] == 0)
			{
				$status = 1;
			}
			else
				$status = 0;
			$data['Tinhchatvanban']['enabled'] = $status;
			
			$data['Tinhchatvanban']['id'] = $id;
	...
	public	function	add()
	{
		...
		{
			if($this->Tinhchatvanban->save($this->request->data))
			{
	...
	public	function	edit($id = null)
	{
		...
		{
			$data = $this->Tinhchatvanban->read(null, $id);
			if(empty($data))
				die('Not found');
			$this->data = $data;
		}
	...
	public	function	delete()
	{
		...
		if(!empty($this->data))
		{
			...
			foreach($ids as $k=>$v)
			{
				if($this->Tinhchatvanban->delete($v))	$success++;
			}
			if($success > 0)
			{
				$this->Session->setFlash('Đã xóa thành công ' . $success . ' mục.', 'flash_success');
				$this->redirect('/tinhchatvanban');
			}
		}
