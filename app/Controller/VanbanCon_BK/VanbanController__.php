...

	{
...

		{
			...
			$van_ban = $this->Vanban->find('first', array('conditions' => array('Filevanban.id' => $id, 
																				'OR' => array('Vanban.nguoi_duyet_id' => $nv1,
...
//		array_push($conds, 'Nhanvanban.ngay_xem IS NULL');
...

	{
		if($this->Auth->User('nhanvien_id') == 683 || $this->Auth->User('nhanvien_id') == 744)//Chị Thu, Anh Dũng
		{
			...
			$ds_phong = $this->Phong->find('list', array('conditions' => array('id' => 102, 'enabled' => 1), 'fields' => array('id', 'id')));
			//pr($ds_phong);die();
			$this->Vanban->recursive = 2;
...

	{
		...
		if(!empty($this->request->data))
		{
			...
			} else //chuyển cho PGĐ duyệt
			{
				...
				$nguoi_duyet = $nguoi_duyet['User']['fullname'];
				$up_date = $this->Vanban->updateAll(
							array('nguoi_duyet_id' => "'" . $nguoi_duyet_id . "'" , 'nguoi_duyet' => "'" . $nguoi_duyet . "'", 'noidung_chuyen' => "'" . $noidung_duyet . "'", 'phongphoihop_id' => "'" . $phongphoihop_id . "'", 'importance' => $importance, 'chuyen_nguoiduyet' => $chuyen_nguoiduyet, 'vb_gap' => $vb_gap, 'vbgap_ngayhoanthanh' => "'".$vbgap_ngayhoanthanh."'"),
							array('Vanban.id' => $id )
								);
			}
...

	{
...

		{
			...
			$this->data = $data;
 			$this->set(compact(array('progress','ds_nhan')));
...
/////////
	...
	public	function	vbkhac_notice()
	{
		...
		$vanban_ids = $this->Nhanvanban->find('all',array('fields' => 'Nhanvanban.vanban_id','conditions' => array(
														...
														"Vanban.tinhtrang_duyet" 	 => 1,
														"Vanban.chieu_di" 				 => 1,
														"CheckNgayVB(Vanban.ngay_duyet,NOW())" => true,
														)));
		//pr($vanban_ids);die();
		foreach($vanban_ids as $kq_vb)
		{
