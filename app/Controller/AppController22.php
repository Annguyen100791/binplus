 ...
 */
	...
	const PHAMVI_VIENTHONG_PHONGBAN_TODOI	= 8;

	const PHAMVI_ALL_TRUNGTAM	= -1;

	const PHAMVI_TRUNGTAM_PHONGBAN_TODOI = 7;
	...
	const	VIENTHONG_PHONGBAN_TODOI = 6;
	...
	const TRUNGTAM_PHONGBAN_TODOI = 5;

	const	ALL_TRUNGTAM = 3;
	...
	public $loai_donvi = array(
		...
		self::VIENTHONG_PHONGBAN_TODOI	=> 'Tổ / Đội thuộc PBCN thuộc viễn thông',


		self::TRUNGTAM			=>	'Trung tâm thuộc viễn thông',


		self::TRUNGTAM_PHONGBAN	=>	'Phòng thuộc Trung tâm',
		self::TRUNGTAM_PHONGBAN_TODOI	=>	'Tổ / Đội thuộc Phòng thuộc Trung tâm',



		self::ALL_TRUNGTAM			=>	'Tất cả trung tâm thuộc viễn thông'
	...
	public	$pham_vi = array(
		...
		self::PHAMVI_VIENTHONG_PHONGBAN_TODOI=>	'Tổ đội thuộc phòng ban chức năng',
		...
		self::PHAMVI_TRUNGTAM_PHONGBAN	=>	'Phòng thuộc Trung tâm',

		self::PHAMVI_TRUNGTAM_PHONGBAN_TODOI	=>	'Tổ / Đội thuộc Phòng thuộc Trung tâm',

		self::PHAMVI_CANHAN		=>	'Cá nhân',

		self::PHAMVI_ALL_TRUNGTAM		=>	'Tất cả trung tâm thuộc viễn thông'
 ...
 */
    ...
    public function beforeFilter() {
        ...
        //Bắt buộc đổi pass định kỳ
	 	$this->loadModel('User');
	 	$date_old = $this->User->field('date_changepass',array('username' => $this->Auth->User('username')));
	 	$day_update = strtotime($date_old);
		$day_now = strtotime(date("Y-m-d H:i:s"));
		$day_check = strtotime("+1 months", $day_update);
		$f = false;
		if ($day_now > $day_check)
		{
			$f = true;

		}
         //if(!is_null( $this->Cookie->read('User') ))
           //if ($day_now > $day_check && $this->request->url != 'users/changepass' && $this->request->url != 'users/login' && $this->request->url != 'users/logout'&& $this->request->url != 'vanban/getfile_ggd')
               //$this->redirect('/users/changepass');
		if ( $this->request->url != 'users/changepass'
		  && $this->request->url != 'users/login'
		  && $this->request->url != 'users/logout'
      && $this->request->url != 'users/apiauthenticate'
		  && strpos($_SERVER['HTTP_USER_AGENT'], 'google') === false )
			if($f)
			{
			  //echo var_dump($this->request->url); die();
			  $this->redirect('/users/changepass');
			}
...

		{
			...
			*/
			...
			//if(!empty($this->Auth->user('username')) && !empty($this->Auth->user('id')))



			// update last action and is_online for users
			//if($this->Auth->user('username') != '' && $this->controller != 'pages' && $this->action != 'check_time')
 ...
 */
...

	{
...

		{
			...
			$ds_phong = $this->Phong->find('list', array('conditions' => array('loai_donvi' => self::VIENTHONG_PHONGBAN, 'enabled' => 1, 'thuoc_phong' => NULL),
														'order' => array('stt' => 'ASC'),
														'fields' => array('id', 'id')));
		...
		}elseif($level == self::PHAMVI_VIENTHONG_PHONGBAN)
		{
			...
			$this->loadModel('Phong');
			$phong_cha =(int)CakeSession::read('Auth.User.phong_cha_id');

			if($phong_cha != NULL){
				$ds_phong_con= $this->Phong->find('list', array('conditions' => array('enabled' => 1, 'thuoc_phong' => $phong_cha,'loai_donvi'=>6), 'fields' => array('id', 'id')));
				if($ds_phong_con!=NULL){
					$ds_phong = implode(",", $ds_phong_con);
					$ds_phong = $ds_phong.','.$phong_cha;
					return $this->Nhanvien->listNhanvien2Combobox($ds_phong);
				}else{
		...
		}elseif($level == self::PHAMVI_ALL_TRUNGTAM)

		{
			$this->loadModel('Phong');

			$ds_phong= $this->Phong->find('list', array('conditions' => array('id ' => array(23,24,25,26,27,28), 'enabled' => 1, 'thuoc_phong' => NULL), 'fields' => array('id', 'id')));
			$ds_phong = implode(",", $ds_phong);
			return $this->Nhanvien->listNhanvien2Combobox($ds_phong);
		...
		elseif($level == self::PHAMVI_TRUNGTAM_PHONGBAN)
		{
			...
			$phong_cha =(int)CakeSession::read('Auth.User.phong_cha_id');
			if($phong_cha != NULL){
				$ds_phong_con= $this->Phong->find('list', array('conditions' => array('enabled' => 1, 'thuoc_phong' => $phong_cha,'loai_donvi'=>5), 'fields' => array('id', 'id')));
				if($ds_phong_con!=NULL){
					$ds_phong = implode(",", $ds_phong_con);
					$ds_phong = $ds_phong.','.$phong_cha;
					return $this->Nhanvien->listNhanvien2Combobox($ds_phong);
...

	{
...

		{
			...
			$ds_phong= $this->Phong->find('list', array('conditions' => array('loai_donvi ' => self::ALL_TRUNGTAM, 'enabled' => 1, 'thuoc_phong' => NULL), 'fields' => array('id', 'id')));
			...
			//pr($this->Nhanvien->getTree_all_ttam($ds_phong)); die();
...

	{
...

		{
			...
			$ds_phong = $this->Phong->find('list', array('conditions' => array('id IN' => array(23,24,25,26,27,28), 'enabled' => 1, 'thuoc_phong' => NULL), 'fields' => array('id', 'id')));
