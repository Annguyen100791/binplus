...
//pr($nv); die();
...

	{
		...
		//	pr($this->request->data['Tinnhan']['nguoinhan_to']); die();
			...
			if(empty($nguoinhan) && empty($this->request->data['Tinnhan']['nguoinhan_to'])){
			...
			if(!empty($nguoinhan)){
				...
				//Loại bỏ người nhận trùng trong $nguoinhan

				if(!empty($this->request->data['Tinnhan']['nguoinhan_to'])){				

					$tam0 = array_diff($tam, $this->request->data['Tinnhan']['nguoinhan_to'] );

					$tam = $tam0;					

				}

				if(!empty($this->request->data['Tinnhan']['nguoinhan_cc'])){				

					$tam1 = array_diff($tam, $this->request->data['Tinnhan']['nguoinhan_cc'] );
				...
				if(!empty($this->request->data['Tinnhan']['nguoinhan_bcc'])){			

					$tam2 = array_diff($tam, $this->request->data['Tinnhan']['nguoinhan_bcc'] );	
				...
				//pr($this->request->data['Chitiettinnhan']).'<br><br>'; die();

								

			}else{

				//Không có người nhận thì bỏ quan hệ			
...
/********************** Hà thêm **************************/
				...
				if(!empty($this->request->data['Tinnhan']['nguoinhan_cc'])){
					...
					//Loai nguoi nhan To khoi CC

					$this->request->data['Tinnhan']['nguoinhan_cc'] = array_diff($this->request->data['Tinnhan']['nguoinhan_cc'], $this->request->data['Tinnhan']['nguoinhan_to']);

					foreach($this->request->data['Tinnhan']['nguoinhan_cc'] as $k => $v)

					{

						//array_push($nguoinhan_cc, array('nguoinhan_id' => $v));
				...
				if(!empty($this->request->data['Tinnhan']['nguoinhan_bcc'])){
					...
					//Loai nguoi nhan To khoi BCC

					$this->request->data['Tinnhan']['nguoinhan_bcc'] = array_diff($this->request->data['Tinnhan']['nguoinhan_bcc'], $this->request->data['Tinnhan']['nguoinhan_cc'], $this->request->data['Tinnhan']['nguoinhan_to']);

					

					foreach($this->request->data['Tinnhan']['nguoinhan_bcc'] as $k => $v)
...
//				//$this->Tinnhan->recursive = -1;
...

			{
				...
				/******Hà thêm để lấy ra ds CC, Bcc*******/

				$this->loadModel('Chitiettinnhan');	

				$dsnhan = $this->Chitiettinnhan->find('all', array(

					'conditions' => array(

						'Chitiettinnhan.tinnhan_id' => $this->passedArgs['replytoall']	,

						'Chitiettinnhan.mark_deleted'	=>	0,

						'Chitiettinnhan.nguoinhan_id <>'	=>	$this->Auth->user('nhanvien_id')	 // Mười thêm để loại bỏ t/h reply all cho chính mình				
			...
			$this->set(compact('tinnhan', 'signature','nguoi_gui','dsnhan'));
	...
	public	function	read_delete()
	{
		...
		if(!empty($this->request->data))
		{
			...
			foreach( $ids as $k => $v )
			{
				//Kiểm tra nếu tin nhắn đã có người đọc
				if($this->Tinnhan->Chitiettinnhan->find('count', 
							array('conditions' => array('tinnhan_id' => $v, 'Chitiettinnhan.ngay_nhan !=' => NULL))) > 0){
					$dadoc++;
				}
				//pr($dadoc); die();
				//Nếu tin nhắn chưa có ai đọc; dadoc==0 thì thực hiện xóa else cảnh báo không cho xóa
				if($dadoc == 0){
			...
			if($daxoa > 0)
			{
				$this->Session->setFlash('Đã xóa thành công ' . $daxoa. ' tin nhắn. Còn '.$chuaxoa.' tin nhắn chưa xóa', 'flash_success');
				$this->redirect('tinnhan/sent');
			}else{
				$this->Session->setFlash('Không có tin nhắn nào đủ điều kiện để xóa', 'flash_error');
				$this->redirect('tinnhan/sent');
			}
		}
	...
	public	function	read_delete_cu_luu()
	{
		if(!$this->check_permission('TinNhan.soanthao'))	// có quyền soạn thảo tin nhắn thì được quyền xóa
			throw new InternalErrorException('Bạn không có quyền xóa tin nhắn. Vui lòng liên hệ quản trị để cấp quyền.');	
	...
	public	function	sent_delete_cu_luu()
	{
		if(!$this->check_permission('TinNhan.soanthao'))
			throw new InternalErrorException('Bạn không có quyền xóa tin nhắn. Vui lòng liên hệ quản trị để cấp quyền.');	
...

	{
		...
		$this->set(compact('nguoinhan','bcc','data'));			
...

		{
			...
			if(empty($nguoinhan) && empty($this->request->data['Tinnhan']['nguoinhan_to'])){
				...
				if(!empty($this->request->data['Tinnhan']['nguoinhan_to'])){				
					...
					$tam1 = array_diff($tam, array($this->request->data['Tinnhan']['nguoinhan_to']) );	
				...
				if(!empty($this->request->data['Tinnhan']['nguoinhan_cc'])){				
					...
					$tam2 = array_diff($tam, $this->request->data['Tinnhan']['nguoinhan_cc'] );	
				...
				if(!empty($this->request->data['Tinnhan']['nguoinhan_bcc'])){			
					...
					$tam3 = array_diff($tam, $this->request->data['Tinnhan']['nguoinhan_bcc'] );	
...
/******************** End Hà thêm *************************************/			
			...
			/******Hà thêm để lấy ra ds CC, Bcc*******/
				...
				$dsnhan = $this->Chitiettinnhan->find('all', array(
					...
					'conditions' => array(
						...
						'Chitiettinnhan.tinnhan_id' => $tinnhan_id	,
		...
		//	pr($tinnhan); die();
			...
			$this->set(compact('tinnhan', 'signature','nguoi_gui','nv','dsnhan'));
	...
	//Thu Hà thêm ngày 07092016

	public	function	mark_theodoi()

	{

		pr($_SERVER["SERVER_NAME"]."/".$_SERVER["SCRIPT_NAME"]); die();

		//var_dump(parse_url($url, PHP_URL_PATH)); die();
...

		{
			...
			{
				if($this->Chitiettinnhan->updateAll(array('theodoi' => 1), array('Chitiettinnhan.id' => $v, 'Chitiettinnhan.nguoinhan_id' => $this->Auth->user('nhanvien_id'))))
		...
		if($f)
			...
			$this->Session->setFlash('Đã đánh dấu thành công ' . $success . ' tin nhắn.', 'flash_success');
		...
		$this->redirect("/".$_SERVER["SERVER_NAME"]."/".$_SERVER["REQUEST_URI"]);
	...
	public	function	unmark_theodoi()

	{
...

		{
...

			{
				...
				if($this->Chitiettinnhan->updateAll(array('theodoi' => 0), array('Chitiettinnhan.id' => $v, 'Chitiettinnhan.nguoinhan_id' => $this->Auth->user('nhanvien_id'))))
		...
		if($f)
			...
			$this->Session->setFlash('Đã bỏ đánh dấu thành công ' . $success . ' tin nhắn.', 'flash_success');

		else

			$this->Session->setFlash('Đã phát sinh lỗi khi bỏ đánh dấu tin nhắn.', 'flash_error');
...

	{
		...
		$conds = array('Chitiettinnhan.theodoi' => 1,
