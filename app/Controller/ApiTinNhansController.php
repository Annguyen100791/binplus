	...
	{	
		...
		{	
			...
			foreach ($data as $n => $v) {
				...
				$num_chitiet = count($v['Chitiettinnhan']) - count($v['Chitiettinnhan'][0]);
		...
		public function listdanhsach(){
			$this->loadModel('User');
			$listdanhsach = array();
			$listdanhsach = $this->User->find('all');
			$res = array();
			$i = 0;
			foreach ($listdanhsach as $value) {
				if(!is_null($value['User']['fullname']) && !is_null($value['Nhanvien']['id']) ){
					$res[$i]['id'] = $value['Nhanvien']['id'];
					$res[$i]['username'] = $value['User']['username'];
					$res[$i]['fullname'] = $value['User']['fullname'];
					$i++;
				}			
			}
		...
		function getname($idnhanvien){
			if(is_numeric($idnhanvien)){
				$this->loadModel('User');
				$dataname = $this->User->find('first',array('conditions' => array('Nhanvien.id' => $idnhanvien)));
				$name = $dataname['User']['username']." - ".$dataname['User']['fullname'];
				return $name;
			}
		}
	}
