...
var Vanban = {
	index:	function(){
		...
		$('#form-gap-search').submit(function(){
			...
			$.ajax({
				type:		'POST',
				url:		BIN.baseURL + 'vanban/vbden_gap',
		...
		$('#form-giamdocchidao-search').submit(function(){
			$('#giamdocchidao-list-content').block({message: BIN.loadingMessage, css: BIN.blockUI});
			$.ajax({
				type:		'POST',
				url:		BIN.baseURL + 'gap/vb_giamdocchidao',
				data:		$('#form-gap-search').serialize(),
				success:	function(result)
				{
					$('#gap-list-content').unblock();
		...
		$('#form-traodoi-search').submit(function(){
			$('#traodoi-list-content').block({message: BIN.loadingMessage, css: BIN.blockUI});
			$.ajax({
				type:		'POST',
				url:		BIN.baseURL + 'vanban/vb_traodoiphanhoi',
				data:		$('#form-traodoi-search').serialize(),
				success:	function(result)
				{
					$('#traodoi-list-content').unblock();
					$('#traodoi-list-content').html(result);
				},
				error:		function(result)
				{
					alert('Error');
	...
	updateTabContent:	function(){
...

		{
			...
			case '#vanban-traodoi':
				...
				BIN.doUpdate('<a href="vanban/vb_traodoiphanhoi" data-target="traodoi-list-content"></a>');

				break;
			case '#vanban-giamdocchidao':

				BIN.doUpdate('<a href="vanban/vb_giamdocchidao" data-target="giamdocchidao-list-content"></a>');
...
index_notice:	function(){
		...
		$(window).hashchange( function(){
			Vanban.updateTabContent_notice();
		})
		$(window).hashchange();
		$('.tu_ngay').datepicker({dateFormat: "dd-mm-yy"});
		$('.den_ngay').datepicker({dateFormat: "dd-mm-yy"});
		...
		$('#form-notice-gap-search').submit(function(){
			$('#notice-gap-list-content').block({message: BIN.loadingMessage, css: BIN.blockUI});
			$.ajax({
				type:		'POST',
				url:		BIN.baseURL + 'vanban/vbgap_notice',
				data:		$('#form-notice-gap-search').serialize(),
				success:	function(result)
				{
					$('#notice-gap-list-content').unblock();
					$('#notice-gap-list-content').html(result);
				},
				error:		function(result)
				{
					alert('Error');
		...
		$('#form-notice-khac-search').submit(function(){
			$('#notice-khac-list-content').block({message: BIN.loadingMessage, css: BIN.blockUI});
			$.ajax({
				type:		'POST',
				url:		BIN.baseURL + 'vanban/vbkhac_notice',
				data:		$('#form-notice-khac-search').serialize(),
				success:	function(result)
				{
					$('#notice-khac-list-content').unblock();
					$('#notice-khac-list-content').html(result);
				},
				error:		function(result)
				{
					alert('Error');
	...
	updateTabContent_notice:	function(){
		var currentTab = window.location.hash;
		if(currentTab == '')
			currentTab = '#notice-gap';
		switch(currentTab)
		{
			case '#notice-gap':
				BIN.doUpdate('<a href="vanban/vbgap_notice" data-target="notice-gap-list-content"></a>');
				break;
			case '#notice-khac':
				BIN.doUpdate('<a href="vanban/vbkhac_notice" data-target="notice-khac-list-content"></a>');
				break;
			default:
				return false;
				break;
...
index_vanbanden:	function(){
		...
		$('#form-ketquakhac-search').submit(function(){
			$('#ketquakhac-list-content').block({message: BIN.loadingMessage, css: BIN.blockUI});
			$.ajax({
				type:		'POST',
				url:		BIN.baseURL + 'vanban/ketquakhac',
				data:		$('#form-ketquakhac-search').serialize(),
				success:	function(result)
				{
					$('#ketquakhac-list-content').unblock();
					$('#ketquakhac-list-content').html(result);
				},
				error:		function(result)
				{
					alert('Error');
		...
		$('#form-pgdduyet-search').submit(function(){
			$('#pgdduyet-list-content').block({message: BIN.loadingMessage, css: BIN.blockUI});
			$.ajax({
				type:		'POST',
				url:		BIN.baseURL + 'vanban/vbden_pgdduyet',
				data:		$('#form-pgdduyet-search').serialize(),
				success:	function(result)
				{
					$('#pgdduyet-list-content').unblock();
					$('#pgdduyet-list-content').html(result);
				},
				error:		function(result)
				{
					alert('Error');
	...
	updateTabContentVBDEN:	function(){
		...
		switch(currentTab)
		{
			...
			case '#vanban-capnhatketqua':
				...
				break;
			case '#vanban-ketquakhac':

				BIN.doUpdate('<a href="vanban/ketquakhac" data-target="ketquakhac-list-content"></a>');

				break;
			case '#vanban-pgdduyet':

				BIN.doUpdate('<a href="vanban/vbden_pgdduyet" data-target="pgdduyet-list-content"></a>');
	...
	add:	function(){
		...
		$('#form-vanban-compose').validate({
			...
			submitHandler: function(form){				
				//console.log($('input[class=uploaded_item]'));
				if($('input[class=uploaded]').length == 0 || $('input[class=uploaded]').val().length < 1)
	...
	trinh_vanban:	function(){
		...
		$('#form-vanban-compose').validate({
			...
			submitHandler: function(form){
				if($("#phongchutri_id").val() == '')
					{
						alert('Vui lòng chọn Đơn vị chủ trì');
						return;
					}
				var nv = $("#nv_selected").val(),
					$donvi = [];
				$.ajax({
					type:		'POST',
					url:		BIN.baseURL + 'vanban/donvi_nv',
					dataType:	'text',
					data:	{nhanvien_id: nv},
					...
					var chutri = $("#phongchutri_id").val();
					console.log(chutri);
						console.log( $donvi);
						
						if($donvi.indexOf(chutri) == -1)
						{
								alert('Vui lòng chọn Nhân viên nhận văn bản thuộc đơn vị chủ trì');
								return;
						}
						if($('#nv_selected').val() == '')
				...
				//Cảnh báo đơn vị chủ trì
				/*var tree = $("#nhanvien-ds").dynatree("getTree");
				var nv_selected_str = $("#nv_selected").val();
				var nv_selected = nv_selected_str.split(",");
				var parent_nv = [];
				for(i = 0; i <= nv_selected.length - 1; i++)
				{
				  var child = tree.getNodeByKey("nv-" + nv_selected[i]);
				  if(child.data.top)
				  {
					while(child.getLevel() > 2)
					{
				  ...
				  if(parent_nv.indexOf(child.data.key) == -1)
				   parent_nv.push(child.data.key);
				}
				var chutri = $("#phongchutri_id").val();
				//console.log(chutri);
				//console.log( parent_nv);
				if(parent_nv.indexOf(chutri) == -1)
				{
						alert('Vui lòng chọn Nhân viên nhận văn bản thuộc đơn vị chủ trì');
						return;
				}*/
	...
	duyet_vanban:	function(){
		...
		$('#btn-vanban-chuyenpgd').click(function(){
	...
	up_result:	function(){
		...
		$('#form-vanban-compose').validate({
			...
			submitHandler: function(form){
				var ds_nhan = $('#ds_nhan').val();
				var ds_chon = $('#nv_selected').val();
				if($('#nv_selected').val() == '')
				...
				var arr_nhan = ds_nhan.split(","),
					arr_chon = ds_chon.split(",");
					//console.log(arr_nhan);
					//console.log(arr_chon);
				if(!array_in_array(arr_nhan,arr_chon))
				{	
					alert('Vui lòng chọn đối tượng để cập nhật kết quả(GĐ VTĐN, CVP, Lãnh đạo duyệt VB này)');
					return false;
				}
				form.submit();
	...
	//////////////  Trao đổi, phản hồi khi xử lý văn bản
	traodoi_phanhoi:	function(){
		...
		$('#form-vanban-compose').validate({
			...
			rules:{
				...
				'data[Vanban][noidung_traodoi]'	:	'required'

			},

			messages:{

				'data[Vanban][noidung_traodoi]'		:	'Vui lòng nhập vào Nội dung cần cập nhật'

			},

			submitHandler: function(form){
				var ds_nhan = $('#ds_nguoinhan').val();
				var ds_chon = $('#nv_selected').val();
				...
				if(!array_in_array(arr_nhan,arr_chon))
				{	
					alert('Vui lòng chọn đối tượng trao đổi thuộc danh sách nhận văn bản');
					return false;
				}
				form.submit();
		...
		BIN.doUpdate('<a href="nhanvien/nhanviennhan/VanBan.traodoi" data-target="vanban_nhanvanban">');		
	...
	add_baocao:	function(){
		...
		new AjaxUpload($('#btn-attachfile'), {
				...
				onSubmit: function(file, ext){
					 ...
					 if (! (ext && /^(doc|docx|xls|xlsx|pdf|rar|tif|tiff|ptt|pptx|odt|ott|odm|html|oth|ods|ots|odg|otg|odp|otp|odf|odb|oxt)$/.test(ext))){ 
						alert('Chỉ cho phép các file TIF, TIFF, DOC, DOCX, XLS, XLSX, PDF, RAR, PPT, PPTX, ODT, OTT, ODM, HTML, OTH, ODS, OTS, ODG, OTG, ODP, OTP, ODF, ODB, OXT');
						return false;
					}
	...
	{
		///////////
		$('#btn-expexcel-search-sua').live('click', function(e){
			e.preventDefault();
			$('#form-vanban-sua').attr('action', BIN.baseURL + 'vanban/excel_search_sua');
			$('#form-vanban-sua').submit();
			$('#form-vanban-sua').attr('action', BIN.baseURL + 'vanban/sua');
			return false;
		});
		//////////
	...
	search_kq:	function(){
		...
		$('#btn-vanban-search').click(function(){
			...
			if($('#form-vanban-search').attr('action') == BIN.baseURL + 'vanban/search_kq')
...

			{
				...
				$.ajax({
					...
					url:		BIN.baseURL + 'vanban/search_kq',
		...
		$('#btn-expexcel-searchkq').live('click', function(e){
			...
			$('#form-vanban-search').attr('action', BIN.baseURL + 'vanban/excel_searchkq');
			...
			$('#form-vanban-search').attr('action', BIN.baseURL + 'vanban/searchkq');
...
// Kiểm tra mảng 2 có trong mảng 1
