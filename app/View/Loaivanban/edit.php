<?php
	echo $this->Form->create('Loaivanban', array('id' => 'form-loaivanban-edit', 'url' => '/loaivanban/edit'));
	echo $this->Form->hidden('id');
?>
<div class="formPanel">    
    <p>
        <?php
			echo $this->Form->input('ten_loaivanban', array('label'	=>	'Tên Tên loại văn bản <span style="color:red">*</span>',
														  'id'		=>	'ten_loaivanban',
														  'style'	=>	'width:97.5%',
														  'class'	=>	'text-input'));
		?>
	...
	$(document).ready(function(){
		$('#form-loaivanban-edit').validate({
			...
			submitHandler: function(form){
				$.ajax({
					type:		'POST',
					url:		BIN.baseURL + 'loaivanban/edit',
					cache:		false,
					async:		false,
					dataType:	'json',
					data:	$('#form-loaivanban-edit').serialize(),
					success:	function(result)
					{
						if(result.success)
						{
							$(form).find('.btn-close').first().click();
							BIN.doUpdate('<a href="loaivanban/index" data-target="loaivanban-list">');
							BIN.doListing();
						}else
