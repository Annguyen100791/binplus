...
<div class="data table-content" style="padding:10px">
	<table>
        ...
        <tr>
            ...
            <!--<th width="0%" hidden="true">Files VB</th> -->
            <th width="0%" hidden="true">ID</th>
            <th>Chọn</th>
        </tr>
        </thead>
    <tbody>
	...
	?>
    	<tr>
           ...
           <!-- <td width="0%" hidden="true">
				<?php 
				foreach($item['Filevanban'] as $file):
					echo $file['ten_cu'];
					echo '<BR>';
                endforeach;
				?>
            ...
            <td width="0%" hidden="true"><?php echo $item['Vanban']['id'] ?></td>
            <td>
...
<script>
	$(document).ready(function(){
		...
		$('.chon-vanban').click(function(){
			...
			//$('#congviec-vanban').html('<div class="uploaded_item"><div style="float:left">' + rows[9].innerHTML + '</div><div style="float:right"><a href="javascript:void(0)" onClick="unselect()"><img src="/img/closelabel.png" title="Bỏ chọn"></a></div><div style="clear:both"></div><input type="hidden" name=data[Congviec][vanban_id] value="' + $(this).attr('rel') + '"></div>');
			$("#id_vanban").val(rows[10].innerText);
