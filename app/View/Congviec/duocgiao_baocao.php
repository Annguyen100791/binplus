	...
	<table class="treeTable">
        ...
        <thead>
            ...
            <tr>
                ...
                <th width="225"><?php echo $this->Paginator->sort('ten_congviec', 'Tên công việc', array('data-mode' => 'ajax', 'data-action' => 'paging', 'data-target' => 'congviec-baocao'));?></th>
 <th width="206"><?php echo $this->Paginator->sort('Vanban.trich_yeu', 'Trích yếu văn bản', array('data-mode' => 'ajax', 'data-action' => 'paging', 'data-target' => 'congviec-all'));?></th>
              <th width="102"><?php echo $this->Paginator->sort('NguoiGiaoviec.full_name', 'Người giao việc', array('data-mode' => 'ajax', 'data-action' => 'paging', 'data-target' => 'congviec-baocao'));?></th>

                <th width="80"><?php echo $this->Paginator->sort('NguoiNhanviec.full_name', 'Người phối hợp', array('data-mode' => 'ajax', 'data-action' => 'paging', 'data-target' => 'congviec-all'));?></th>
                <th width="40"><?php echo $this->Paginator->sort('ngay_batdau', 'Ngày bắt đầu', array('data-mode' => 'ajax', 'data-action' => 'paging', 'data-target' => 'congviec-baocao'));?></th>

                <th width="42"><?php echo $this->Paginator->sort('ngay_ketthuc', 'Ngày hoàn thành', array('data-mode' => 'ajax', 'data-action' => 'paging', 'data-target' => 'congviec-baocao'));?></th>
				<th width="66"><?php echo $this->Paginator->sort('loaicongviec_id', 'Loại công việc', array('data-mode' => 'ajax', 'data-action' => 'paging', 'data-target' => 'congviec-all'));?></th>

                <th width="46"><?php echo $this->Paginator->sort('mucdo_hoanthanh', 'Hoàn thành', array('data-mode' => 'ajax', 'data-action' => 'paging', 'data-target' => 'congviec-baocao'));?></th>

                <th width="82">Thao tác</th>
            </tr>
        </thead>
                ...
                <td>
                    ...
                    <?php 
						...
						if($item['Congviec']['nguoinhan_id'] == $this->Session->read('Auth.User.nhanvien_id'))
							...
							echo $this->Html->link($item['Congviec']['ten_congviec'], '/congviec/view/' . $item['Congviec']['id'] .'/congviec-baocao', array('title' => '<b>' . $item['NguoiGiaoviec']['full_name'] . '</b> đã giao cho bạn <p><b>Tên công việc:</b> ' . $item['Congviec']['ten_congviec'] . '<BR><b>Nội dung:</b> ' . $item['Congviec']['noi_dung'] . '<BR><b>Ngày giao việc:</b> ' . $this->Time->format('d-m-Y H:i:s', $item['Congviec']['ngay_giao']) . '</p>Click để xem chi tiết công việc'));
...
<td><?php 
                ...
                <?php
...

					{
						 ...
						 echo $this->Html->link($this->Html->image('icons/time.png', array('class' => 'icon-button')), '/congviec/update_progress/' . $item['Congviec']['id'].'/congviec-baocao', array('title' => 'Cập nhật mức độ hoàn thành công việc', 'escape' => false, 'data-mode' => 'ajax', 'data-action' => 'dialog'));
    ...
    <tfoot>
    	...
    	<tr>
            ...
            <div style="float:left; padding: 20px 0">
        		...
        		<a href="/congviec/mark_read/type:baocao" class='button' data-mode="ajax" data-action="delete-all"  data-msg="Vui lòng chọn trong danh sách" data-confirm="Chức năng này để cập nhật tiến độ 100% những công việc không liên quan, bạn có đồng ý cập nhật không ?" data-target="congviec-baocao">Đánh dấu </a>    		</div>            </td>
            ...
            <td colspan="8">
            	...
            	<div class="pagination">
                	...
                	[Trang <?php echo $this->Paginator->counter(); ?> ] &nbsp;
                        ...
                        <?php echo $this->Paginator->first('|<', array('class' => 'number', 'data-mode' => 'ajax', 'data-action' => 'paging', 'data-target' => 'congviec-baocao'))?>

                        <?php echo $this->Paginator->prev('<', array('class' => 'number', 'data-mode' => 'ajax', 'data-action' => 'paging', 'data-target' => 'congviec-baocao'))?>

                        <?php echo $this->Paginator->numbers(array('separator'=>'&nbsp;', 'class' => 'number', 'data-mode' => 'ajax', 'data-action' => 'paging', 'data-target' => 'congviec-baocao')); ?>

                        <?php echo $this->Paginator->next('>', array('class' => 'number', 'data-mode' => 'ajax', 'data-action' => 'paging', 'data-target' => 'congviec-baocao'))?>

                        <?php echo $this->Paginator->last('>|', array('class' => 'number', 'data-mode' => 'ajax', 'data-action' => 'paging', 'data-target' => 'congviec-baocao'))?>                </div>
