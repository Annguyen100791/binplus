...
<div class="content-box-header"> <!-- Add the class "closed" to the Content box header to have it closed by default -->
    <div style="float:left">
        <h3><?php echo $this->Html->image('icons/user.png', array('align' => 'left')) ?>&nbsp; Định hướng giao văn bản</h3>
    </div>
...
//			idPrefix: "dynatree-Cb3-",
		...
		var oldCheckedNode = -99;
		 /*$('#phongchutri_id').on('change',function(e){
			var value = $(this).val();
			var tree = $("#nhanvien-ds").dynatree("getTree");
			if(typeof tree == 'undefined')
			  return;
			if(oldCheckedNode != value && oldCheckedNode != -99 && folderNode.getLevel() <= 2)
			{
				tree.getNodeByKey(oldCheckedNode).visit(function(node){
					node.select(false);
				});
			}
			var folderNode = tree.getNodeByKey(value);
			console.log(folderNode);
			checkTree(true,folderNode);
			oldCheckedNode = value;
		  });*/
		  $('#phongchutri_id').on('change',function(e){
			var value = $(this).val();
			var tree = $("#nhanvien-ds").dynatree("getTree");
			...
			if(typeof tree == 'undefined')
			  return;
			//console.log(folderNode.getLevel());
			if(oldCheckedNode != value && oldCheckedNode != -99 && folderNode.getLevel() <= 2)
			{
				tree.getNodeByKey(oldCheckedNode).visit(function(node){
					node.select(false);
